...
/* Boot-time self tests (CONFIG_SELFTEST). Run after all subsystems are up,
 * before init starts. */
#include "zaeboot.h"
#include "printf.h"
#include "string.h"
...
static void test_sched(void)
{
    ...
    spin_done = 1;
    uint32_t sp_id = sp->id;                /* `sp` is freed once reaped */
    task_join(sp_id);
    task_sleep_ms(5);                       /* let idle reap the zombie */
    CHECK(!task_alive(sp_id), "spinner still alive after join");
...
{
#ifndef CONFIG_SMP
    kprintf("smp: not configured, skipping\n");
    return;
#endif
    if (smp_cpu_count() < 2) {
    ...
    uint32_t ids[4];
    for (int i = 0; i < 4; i++)
        ids[i] = task_create("smp-spin", spinner, NULL)->id;
    task_sleep_ms(100);
    spin_done = 1;
    for (int i = 0; i < 4; i++)
        task_join(ids[i]);
    ...
    uint32_t a = task_create("heap-a", (task_entry_t)test_heap_worker, NULL)->id;
    uint32_t b = task_create("heap-b", (task_entry_t)test_heap_worker, NULL)->id;
    task_join(a);
    task_join(b);
...
static void test_user(void)
{
    ...
    kprintf("user: hypervisor '%s'\n", hv[0] ? hv : "none (bare metal)");
    struct task *crash = process_spawn("/bin/crash", NULL, envp);
    struct task *test  = process_spawn("/bin/test", NULL, envp);
